(function () {
    ...
    // 槽位颜色值：纯色 '#rrggbb'；渐变 { g:true, from:'#..', to:'#..', d:135(角度,0=朝上 顺时针) }
    function isGradVal(v) { return !!(v && typeof v === 'object' && v.g); }
    function gradVal(v, def) {
        if (isGradVal(v)) return { g: true, from: v.from || def, to: v.to || def, d: (v.d == null ? 135 : v.d) };
        return { g: false, from: (typeof v === 'string' ? v : def) || def, to: def, d: 135 };
    }
    function paintCss(v, def) {
        var gv = gradVal(v, def);
        return gv.g ? ('linear-gradient(' + gv.d + 'deg,' + gv.from + ',' + gv.to + ')') : gv.from;
    }
    function colorNeedsTint(v, def) {
        if (!v) return false;
        if (isGradVal(v)) return true;
        return typeof v === 'string' && v.toLowerCase() !== String(def).toLowerCase();
    }
    ...
    function defaultEdit(t) {
        return {
            ...
            persons: [], stickers: [], slotImgs: {}, slotTexts: {}, slotColors: {}
        };
    }
    ...
    function slotHtml(sl, src, uiOff, preview, ed) {
        ...
        if (sl.shape === 'thumb' && sl.tint) {
            var tg = (ed && ed.slotColors && ed.slotColors[sl.id]) || {};
            var pad = 100.8 - sl.w / 2;
            var cw = sl.w + 2 * pad, ch = sl.h + 2 * pad;
            var customImg = ed && ed.slotImgs && ed.slotImgs[sl.id];
            var bgCss = paintCss(tg.bg, sl.tint.bg);
            var bgSpan = '<span class="slot-tint-bg" style="left:-' + pad + 'px;top:-' + pad + 'px;width:' + cw + 'px;height:' + ch + 'px;background:' + bgCss + '"></span>';
            var tInner;
            if (customImg) {
                tInner = '<img class="slot-tint-img" src="' + customImg + '" alt="" style="left:-' + pad + 'px;top:-' + pad + 'px;width:' + cw + 'px;height:' + ch + 'px">';
            } else {
                var fgv = gradVal(tg.fg, sl.tint.fg);
                var needTint = colorNeedsTint(tg.fg, sl.tint.fg);
                var colVal = fgv.g ? { g: true, from: fgv.from, to: fgv.to, d: fgv.d } : fgv.from;
                var dataCol = needTint ? (' data-gcol="' + esc(JSON.stringify(colVal)) + '"') : ' data-gcol=""';
                tInner = '<img class="slot-tint-glyph" src="' + sl.src + '" alt="" data-gsrc="' + sl.src + '"' + dataCol + ' data-gdef="' + sl.tint.fg + '">';
            }
            return '<div class="p-slot p-slot-thumb p-slot-tint p-chrome' + edc + (uiOff ? ' off' : '') + '"' + ds + ' style="' + base + '">'
                + bgSpan + tInner + '</div>';
        }
        if (sl.shape === 'thumb') {
        ...
        if (sl.shape === 'card' && sl.tint) {
            var cg = (ed && ed.slotColors && ed.slotColors[sl.id]) || {};
            var cardCss = paintCss(cg.card, sl.tint.card);
            var cardChanged = colorNeedsTint(cg.card, sl.tint.card);
            var agv = gradVal(cg.arrow, sl.tint.arrow);
            var arrowChanged = colorNeedsTint(cg.arrow, sl.tint.arrow);
            var showCard = src || cardChanged || arrowChanged;
            var bodySt = (cardChanged || arrowChanged) ? ' style="background:' + cardCss + '"' : '';
            var ringSt = '';
            if (arrowChanged) {
                ringSt = agv.g
                    ? ' style="background:linear-gradient(' + agv.d + 'deg,' + agv.from + ',' + agv.to + ')"'
                    : ' style="background:' + hexA(agv.from, 0.35) + '"';
            }
            var iSt = arrowChanged ? ' style="color:' + agv.from + '"' : '';
            return '<div class="p-slot p-slot-card' + edc + (showCard ? '' : emc) + '"' + ds + ' style="' + base + '">'
                + (showCard
                    ? '<div class="slot-card-body"' + bodySt + '>' + (src ? '<img class="slot-img" src="' + src + '" alt="">' : '') + '</div>'
                        + '<div class="slot-arrow"' + ringSt + '><i' + iSt + '>→</i></div>'
                    : '')
                + '</div>';
        }
    ...
    function buildPoster(tpl, ed, sel, preview) {
        ...
        (tpl.slots || []).forEach(function (sl) {
            if (sl.shape === 'text' || sl.shape === 'prompt') {
                ...
                if (src || !preview) html += slotHtml(sl, src || '', !ed.ui, preview, ed);
            }
        });
    ...
    function renderPoster() {
        wrap.innerHTML = '';
        var node = buildPoster(getTpl(state.tpl), cur(), state.sel);
        wrap.appendChild(node);
        applyGlyphTints(node);
        fit();
    }
    ...
    function buildSlotList() {
        ...
        slots.forEach(function (sl) {
            ...
            row.className = 'pe-slot-row' + (sl.tint ? ' pe-slot-tintrow' : '');
            ...
            if (sl.tint) {
                ...
                box.className = 'pe-tintbox';
                var tc = (ed.slotColors && ed.slotColors[sl.id]) || {};
                var fields = sl.shape === 'card'
                    ? [['card', '卡片底色'], ['arrow', '箭头颜色']]
                    : [['bg', '图标底色'], ['fg', '图标颜色']];
                var DIRS = [135, 180, 90, 0, 270, 225, 315, 45];
                var DIR_ICON = { 0: '↑', 45: '↖', 90: '→', 135: '↘', 180: '↓', 225: '↙', 270: '←', 315: '↗' };
                var tintChanged = fields.some(function (fd) { return colorNeedsTint(tc[fd[0]], sl.tint[fd[0]]); });
                function ensureTint() {
                    if (!ed.slotColors) ed.slotColors = {};
                    if (!ed.slotColors[sl.id]) ed.slotColors[sl.id] = {};
                    return ed.slotColors[sl.id];
                }
                fields.forEach(function (fd) {
                    var key = fd[0], label = fd[1];
                    var gv = gradVal(tc[key], sl.tint[key]);
                    var field = document.createElement('div');
                    field.className = 'pe-tint-field';

                    var lab1 = document.createElement('label');
                    lab1.className = 'pe-tint-item';
                    var dot1 = document.createElement('span');
                    dot1.className = 'pe-tint-dot';
                    dot1.style.background = gv.g ? ('linear-gradient(' + gv.d + 'deg,' + gv.from + ',' + gv.to + ')') : gv.from;
                    var tx1 = document.createElement('span');
                    tx1.className = 'pe-tint-label';
                    tx1.textContent = label;
                    var in1 = document.createElement('input');
                    in1.type = 'color';
                    in1.className = 'pe-tint-in';
                    in1.value = /^#[0-9a-fA-F]{6}$/.test(gv.from) ? gv.from : '#ffffff';
                    lab1.appendChild(dot1); lab1.appendChild(tx1); lab1.appendChild(in1);
                    field.appendChild(lab1);

                    var gbtn = document.createElement('button');
                    gbtn.type = 'button';
                    gbtn.className = 'pe-tint-gradbtn' + (gv.g ? ' on' : '');
                    gbtn.textContent = gv.g ? '纯色' : '渐变';
                    field.appendChild(gbtn);

                    var gradWrap = document.createElement('span');
                    gradWrap.className = 'pe-tint-grad' + (gv.g ? ' on' : '');
                    var lab2 = document.createElement('label');
                    lab2.className = 'pe-tint-item';
                    var dot2 = document.createElement('span');
                    dot2.className = 'pe-tint-dot pe-tint-dot2';
                    dot2.style.background = gv.to;
                    var in2 = document.createElement('input');
                    in2.type = 'color';
                    in2.className = 'pe-tint-in';
                    in2.value = /^#[0-9a-fA-F]{6}$/.test(gv.to) ? gv.to : '#ffffff';
                    lab2.appendChild(dot2); lab2.appendChild(in2);
                    gradWrap.appendChild(lab2);
                    var dirbtn = document.createElement('button');
                    dirbtn.type = 'button';
                    dirbtn.className = 'pe-tint-dir';
                    dirbtn.textContent = DIR_ICON[gv.d] || '↘';
                    dirbtn.title = '渐变方向';
                    gradWrap.appendChild(dirbtn);
                    field.appendChild(gradWrap);

                    function liveGv() { return gradVal(((ed.slotColors && ed.slotColors[sl.id]) || {})[key], sl.tint[key]); }
                    in1.addEventListener('input', function () {
                        var c = liveGv();
                        if (c.g) {
                            ensureTint()[key] = { g: true, from: this.value, to: c.to, d: c.d };
                            dot1.style.background = 'linear-gradient(' + c.d + 'deg,' + this.value + ',' + c.to + ')';
                        } else {
                            ensureTint()[key] = this.value;
                            dot1.style.background = this.value;
                        }
                        renderPoster();
                    });
                    in1.addEventListener('change', function () { buildSlotList(); });
                    in2.addEventListener('input', function () {
                        var c = liveGv();
                        ensureTint()[key] = { g: true, from: c.from, to: this.value, d: c.d };
                        dot2.style.background = this.value;
                        dot1.style.background = 'linear-gradient(' + c.d + 'deg,' + c.from + ',' + this.value + ')';
                        renderPoster();
                    });
                    in2.addEventListener('change', function () { buildSlotList(); });
                    gbtn.addEventListener('click', function () {
                        var c = liveGv();
                        if (c.g) ensureTint()[key] = c.from;
                        else ensureTint()[key] = { g: true, from: c.from, to: c.from, d: 135 };
                        renderPoster();
                        buildSlotList();
                    });
                    dirbtn.addEventListener('click', function () {
                        var c = liveGv();
                        var idx = DIRS.indexOf(c.d);
                        var nd = DIRS[(idx + 1) % DIRS.length];
                        ensureTint()[key] = { g: true, from: c.from, to: c.to, d: nd };
                        renderPoster();
                        buildSlotList();
                    });
                ...
                if (tintChanged) {
                    ...
                    rstT.type = 'button';
                    rstT.className = 'pe-btn pe-btn-sm pe-btn-ghost pe-tint-reset';
                    rstT.textContent = '还原颜色';
                    rstT.addEventListener('click', function () {
                        if (ed.slotColors) delete ed.slotColors[sl.id];
                        renderPoster();
                        buildSlotList();
                    });
                    box.appendChild(rstT);
                }
                row.appendChild(box);
            }
    ...
    // monochrome 图标 PNG 重着色（保留 alpha），支持纯色/线性渐变；结果缓存；屏幕显示与 html2canvas 导出通用
    var glyphTintCache = {};
    function tintedGlyph(src, val) {
        var key = src + '|' + JSON.stringify(val);
        if (glyphTintCache[key]) return Promise.resolve(glyphTintCache[key]);
        return new Promise(function (resolve) {
            ...
            im.onload = function () {
                try {
                    ...
                    cx.drawImage(im, 0, 0);
                    var gv = gradVal(isGradVal(val) ? val : (typeof val === 'string' ? val : null), '#ffffff');
                    if (gv.g) {
                        var rad = (gv.d - 90) * Math.PI / 180;
                        ...
                        grad.addColorStop(0, gv.from);
                        grad.addColorStop(1, gv.to);
                        cx.globalCompositeOperation = 'source-in';
                        cx.fillStyle = grad;
                        cx.fillRect(0, 0, w, h);
                    } else {
                        var d = cx.getImageData(0, 0, w, h);
                        var hh = gv.from.replace('#', '');
                        if (hh.length === 3) hh = hh[0] + hh[0] + hh[1] + hh[1] + hh[2] + hh[2];
                        var n = parseInt(hh, 16);
                        var r = (n >> 16) & 255, gg = (n >> 8) & 255, bb = n & 255;
                        for (var i = 0; i < d.data.length; i += 4) {
                            d.data[i] = r; d.data[i + 1] = gg; d.data[i + 2] = bb;
                        }
                        cx.putImageData(d, 0, 0);
                    }
                    var url = c.toDataURL('image/png');
                    glyphTintCache[key] = url;
                    resolve(url);
    ...
    function applyGlyphTints(root) {
        var imgs = root.querySelectorAll('img.slot-tint-glyph');
        var ps = [];
        Array.prototype.forEach.call(imgs, function (im) {
            if (im.dataset.gcol) {
                var val;
                try { val = JSON.parse(im.dataset.gcol); } catch (e) { val = null; }
                if (val) {
                    ps.push(tintedGlyph(im.dataset.gsrc, val).then(function (url) {
                        if (url && im.isConnected) im.src = url;
                    }));
                }
            }
        });
    ...
    function exportPNG() {
        ...
        document.fonts.ready.then(function () {
            ...
            return applyGlyphTints(node).then(function () { return waitImages(node); }).then(function () {
